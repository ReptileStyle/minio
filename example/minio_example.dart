...
void main() async {
  ...
  final poller = Minio.shared.listenBucketNotification(bucket, events: [
    's3:ObjectCreated:*',
  ],);
  ...
  final url = await Minio.shared.presignedGetObject(bucket, object, expires: 1000);
  log('--- presigned url:');
  log(url);

  final copyResult1 = await Minio.shared.copyObject(bucket, copy1, '$bucket/$object');
  final copyResult2 = await Minio.shared.copyObject(bucket, copy2, '$bucket/$object');
